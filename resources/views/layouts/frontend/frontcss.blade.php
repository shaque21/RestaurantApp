<link rel="stylesheet" href="{{ asset('components/assets') }}/css/bootstrap.min.css" />
    <link
      rel="stylesheet"
      href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.4.1/font/bootstrap-icons.css"
    />
    
    {{-- <link rel="stylesheet" href="https://unpkg.com/aos@next/dist/aos.css" /> --}}

    <link rel="stylesheet" href="{{ asset('components/assets') }}/css/fstdropdown.min.css">
    <link rel="stylesheet" href="{{ asset('components/assets') }}/css/front-style.css" />
    <script src="{{ asset('components/assets') }}/js/jquery-3.5.1.js"></script>
    <script src="{{ asset('components/assets') }}/js/sweetalert2@11.js"></script>

    {{-- <script src="{{ asset('components/assets') }}/js/sweetalert.min.js"></script> --}}
